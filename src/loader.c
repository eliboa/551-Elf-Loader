...
void _start()
{
	...
	if(valid_elf_image() != 1)
		OSFatal("elf image isn't valid");
...
int write_data(void *buffer, int size, int nmemb, void *userp)
{
	...
	if((*filepos)+insize > 0x7FFFFC)
		OSFatal("filepos is rip");
...
void myMemThread(int sth1, int *sth2)
{
	...
	*(int*)0xF5FFFFFC = 0;
	...
	if(!datalen || datalen > 0x7FFFFC)
		OSFatal("datalen is greater than 0x7FFFFC");
	
	int resp = 404;
	curl_easy_getinfo(curl_handle, CURLINFO_RESPONSE_CODE, &resp);
	if(resp != 200)
		OSFatal("resp is not 200");
